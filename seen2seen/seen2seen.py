...
# usage:
# mode train and test: assurer dans config.cfg : annotation_ONLY = False , ensuite:
# python seen2seen.py
# mode annotation: assurer dans config.cfg : annotation_ONLY = True , mettre le corpus dans INPUT/LANG, ensuite:
...
def obtentionCheminCorpus(languesAtraiter):
    ...
    for langue in languesAtraiter :
        ...
        # Choix de la colonne comportant les POS au format UD ou à défaut les POS les plus détaillées
    ...
    # Ajoute dans le fichier de config la colonne de POS qui sera prise en compte pour chaque langue
...
def generation_cands_parCombinaison(dicoCands) :
    ...
    OUTPUT_seen = dico avec strings des combinaisons triees par ordre croissant de tokens : le;occasion;être': ['4|13|23'....
    """
    new_dicoCand = {}
    for numPhrase in dicoCands :
                                ...
                                for comptMWE in comptMWEs :
                                    if int(infos[0]) in toks2annot_categ :
                                        if comptMWE not in premiers_toks_annot :
                                            premiers_toks_annot.append(comptMWE) # car ajote info catégorie uniquement la 1ere fois
...
for langue in pbar :
    ...
    if annotation_ONLY: ### CHANGEMENT : ajout condition
        ...
        if len(sys.argv) > 1:
            ...
            print("ERREUR")
            print("Vous êtes en mode annotation_ONLY : changez de configuration ou entrez un corpus en argument.")
            print(f"Le corpus doit être un répertoire et se trouver dans Seen2Seen/INPUT/{langue}.")
            sys.exit()
